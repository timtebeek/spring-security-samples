...
This library provides us with `SecurityMockMvcRequestPostProcessors#jwt()`, among others, which we add to our test methods to have them propagate a JWT with each request.
[source,java]
----
...
Because of this, we do have to add `csrf` tokens to our POST requests, which is provided through a `CsrfRequestPostProcessor` from `spring-security-test`.

With both `get("...").with(jwt())` and `post("...").with(jwt()).with(csrf())` in place, now all tests within `LeaveRequestControllerWebMvcTest` pass again!
