...
While this is all fine and needed; it does not yet achieve much in terms of security; anyone can still authenticate, file and view leave requests and approve or deny them as they see fit.
We need to configure our application with some common sense roles and restrictions.
For instance: we only want users to submit and view requests for themselves; while on HR employees can approve or deny requests.
...
We've added a variety of security annotation and expressions, all of which are documented extensively in https://docs.spring.io/spring-security/site/docs/5.2.x/reference/html/authorization.html[the Authorization chapter of Spring Security].
The security annotations are enabled through `@EnableGlobalMethodSecurity(prePostEnabled = true, jsr250Enabled = true)`.
