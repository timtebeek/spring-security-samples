...
We can achieve this fairly easily by providing a `@MockBean JwtDecoder`, which by passes the call to our `issuer-uri`, https://docs.spring.io/spring-security/site/docs/5.2.x/reference/htmlsingle/#literal-authentication-literal-literal-requestpostprocessor-literal[as alluded to in the documentation].
When we add such a field to our tests we can see that they now pass again, even when our OpenID Connect provider is unavailable.
